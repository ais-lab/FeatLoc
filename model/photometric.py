""" original from photometric augmentation in SuperPoint Paper https://arxiv.org/abs/1712.07629
# used in dataloader 
"""
    ...
    def __init__(self, **config):
        ...
        if config['photometric']['enable']:
            ...
            if params.get('random_contrast', False):
                ...
                aug = iaa.LinearContrast((change[0], change[0]))
                aug_all.append(aug)
            ...
            if params.get('additive_gaussian_noise', False):
                ...
                aug = iaa.AdditiveGaussianNoise(scale=(change[0], change[0]))
                aug_all.append(aug)
            ...
            if params.get('additive_speckle_noise', False):
                ...
                aug = iaa.ImpulseNoise(p=(change[0], change[0]))
                aug_all.append(aug)
            # if params['motion_blur']:
            if params.get('motion_blur', False):
                change = params['motion_blur']['max_kernel_size']
                # if change > 3:
                    #change = randint(change, change)
                aug = iaa.MotionBlur(change)
                # elif change == 3:
...
#             kernel_size = kernel_size_range[0]
